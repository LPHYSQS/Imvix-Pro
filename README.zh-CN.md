...
Imvix Pro 是一个以 Windows 为主要落地平台的桌面转换工具，当前仓库版本为 `2.0.1`。
`2.0.1` 为一次界面与文案同步更新，主要新增越南语（`vi-VN`）与泰语（`th-TH`）支持，并同步更新了版本展示、更新说明和多语言文档内容。
...
当前内置以下 12 种界面语言资源（`2.0.1` 新增 `vi-VN` 与 `th-TH`）：
