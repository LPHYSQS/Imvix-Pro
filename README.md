...
Imvix Pro 的项目源码现已在 GitHub 仓库中公开发布，并依据附带商业使用限制的自定义许可提供。

- 个人开发、学习研究、评估测试、非商业修改与再分发，可在遵守 [`LICENSE`](LICENSE) 的前提下进行
- 任何个人或组织如拟将本项目用于商业产品、付费服务、营收流程、企业内部商业场景或其他商业用途，必须事先取得作者或版权持有人的书面授权
- 商业授权联系邮箱：`339106817@qq.com`

## 项目概览

Imvix Pro 是一个以 Windows 为主要落地平台的桌面转换工具，当前仓库版本为 `2.0.2`。
`2.0.2` 主要聚焦 OCR 识别体验优化与语言能力扩展，修复 OCR 结果文本区域高度自适应异常问题，并新增对越南语（`vi-VN`）与泰语（`th-TH`）的基础识别支持。
...
| 本地化与体验 | 支持 12 种界面语言、亮色/暗色/跟随系统主题、窗口位置记忆、PDF 锁定提示与解锁流程 |
...
当前内置以下 12 种界面语言资源（含 `vi-VN` 与 `th-TH`）：
...
本项目现已公开源代码，具体许可条款见 [`LICENSE`](LICENSE)。

- 在遵守许可条款的前提下，您可以查阅、使用、修改并以非商业目的再分发本项目的源码与二进制
- 仓库公开并不构成对商业使用的默认授权；除作者 / 版权持有人外，任何个人或组织如需将本项目用于商业产品、付费服务、营收流程、企业内部商业场景或其他商业用途，必须事先取得书面许可
